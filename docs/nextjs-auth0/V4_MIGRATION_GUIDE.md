# Auth0 NextJS v4 Setup Guide

This guide covers the setup and usage of `@auth0/nextjs-auth0` v4 for the UniCal project.

> **Note:** This project uses Auth0 v4 directly - no migration from v3 is needed.

## Environment Variables

The following environment variables are required for Auth0 v4:
...
- `APP_BASE_URL`: Your application's base URL (e.g.: `http://localhost:3000`)
- `AUTH0_DOMAIN`: Your Auth0 domain without the scheme (e.g.: `example.us.auth0.com`)
- All other configuration must be specified via the `Auth0Client` constructor

> [!NOTE]
> The `audience` parameter must be specified as a query parameter or via the `authorizationParameters` configuration option. For more information, see [Passing custom authorization parameters](#passing-custom-authorization-parameters).

## Route Setup

In Auth0 v4 with Next.js App Router, you need to create a Route Handler under `/app/api/auth/[auth0]/route.ts`:
...
In Auth0 v4, the `<UserProvider />` has been renamed to `<Auth0Provider />` and **is no longer required by default**.

If you would like to pass an initial user during server rendering to be available to the `useUser()` hook, you can wrap your components with the `<Auth0Provider />` ([see example](https://github.com/auth0/nextjs-auth0/blob/main/EXAMPLES.md#auth0provider-)).
